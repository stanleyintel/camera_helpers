...
PATCH_1_SRC="$SCRIPT_DIR/0002-media-ipu7-sync-acpi-csi2-config-abi.patch"
LOCAL_BRANCH="ref_main/8c83b94"
REMOTE_COMMIT_ID="8c83b94"
...
if [[ ! -f "$PATCH_1_SRC" ]]; then
  echo "ERROR: patch not found: $PATCH_1_SRC" >&2
  exit 1
fi

run git clone "$REPO_URL" "$DRIVER_DIR"
run git -C "$DRIVER_DIR" checkout -B $LOCAL_BRANCH $REMOTE_COMMIT_ID
...
apply_patch_if_needed "$PATCH_1_SRC"
