...
WORK_DIR="$(pwd)"
REPO_DIR="$WORK_DIR/ipu7-camera-bins"

if [[ ! -d "$REPO_DIR" ]]; then
  echo "Missing ipu7-camera-bins. Run ./1_do_clone_sources.sh first." >&2
  exit 1
fi
...
if ((${#runtime_libs[@]} == 0 || ${#firmware_bins[@]} == 0 || ${#pkgconfigs[@]} == 0)); then
  echo "ipu7-camera-bins layout does not match the expected 26Q1.2 release structure." >&2
  exit 1
fi
...
run sudo mkdir -p /usr/include/ipu7 /usr/lib/pkgconfig
