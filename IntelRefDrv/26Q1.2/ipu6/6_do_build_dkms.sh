...
run sudo -E dkms uninstall ipu-camera-sensor/0.1 || true
run sudo -E dkms remove ipu-camera-sensor/0.1 || true
...
if $DRY_RUN; then
  run sudo -E dkms status -m ipu-camera-sensor -v 0.1
  echo "dry-run: skipped dkms build/install verification"
  exit 0
fi

DKMS_STATUS="$(sudo -E dkms status -m ipu-camera-sensor -v 0.1 || true)"
echo "$DKMS_STATUS"
